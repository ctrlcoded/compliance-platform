...
describe('PoolAllocator (Article 21)', () => {
    it('correctly allocates surplus to deficits based on greedy algorithm', () => {
        const ships: PoolShip[] = [
            { shipId: 'S1', cbBefore: cb(5000) },
            { shipId: 'S2', cbBefore: cb(-3000) },
            { shipId: 'S3', cbBefore: cb(-1000) },
        ];
        ...
        expect(result.find((s) => s.shipId === 'S1')?.cbAfter.value).toBe(1000); // 5000 - 3000 - 1000
        expect(result.find((s) => s.shipId === 'S2')?.cbAfter.value).toBe(0);
        expect(result.find((s) => s.shipId === 'S3')?.cbAfter.value).toBe(0);
    });

    it('throws NEGATIVE_POOL_SUM when sum is < 0', () => {
        const ships: PoolShip[] = [
            { shipId: 'S1', cbBefore: cb(2000) },
            { shipId: 'S2', cbBefore: cb(-3000) }, // sum = -1000
        ];
    ...
    it('maintains rules for deficit ship protection and surplus ship protection', () => {
        const ships: PoolShip[] = [
            { shipId: 'S1', cbBefore: cb(500) },
            { shipId: 'S2', cbBefore: cb(-200) },
            { shipId: 'S3', cbBefore: cb(-300) },
        ];

        const result = PoolAllocator.allocate(ships);

        // S1=0, S2=0, S3=0
        expect(result.find((s) => s.shipId === 'S1')?.cbAfter.value).toBeGreaterThanOrEqual(0); // Surplus protection
        expect(result.find((s) => s.shipId === 'S2')?.cbAfter.value).toBeGreaterThanOrEqual(-200); // Deficit protection
    });
});
