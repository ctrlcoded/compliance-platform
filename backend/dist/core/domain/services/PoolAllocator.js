...
class PoolAllocator {
     ...
     */
    static allocate(ships) {
        if (ships.length < 2) {
            throw new DomainError_1.DomainError('A pool must contain at least 2 ships', 'INVALID_POOL_SIZE');
        }
        let sum = 0;
        const deficits = [];
        const surpluses = [];
        ...
        if (this.round4(sum) < 0) {
            throw new DomainError_1.DomainError('Pool total compliance balance cannot be negative', 'NEGATIVE_POOL_SUM');
        }
        ...
        while (dIdx < currentDeficits.length && sIdx < currentSurpluses.length) {
            ...
            if (this.round4(currentDeficits[dIdx].remainingRequired) === 0)
                dIdx++;
            if (this.round4(currentSurpluses[sIdx].remaining) === 0)
                sIdx++;
        }
